...
function collectRefs(
    ...
    if (def.$ref && isLocalRef(def.$ref)) {
        const name = decodeURIComponent(def.$ref.slice(DEFINITION_OFFSET));
    ...
    if (Array.isArray(def.type) ? def.type.includes("object") : def.type === "object") {
        if (def.properties) {
    ...
    if (Array.isArray(def.type) ? def.type.includes("array") : def.type === "array") {
...
export function inlineSingleUseDefs(
    ...
    while (changed) {
        ...
        for (const [name, count] of counts) {
            if (count === 1 && defs[name] && isGenerated(name)) {
                toInline.add(name);
            }
        }
        if (toInline.size === 0) {
